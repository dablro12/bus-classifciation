...
def plot_slope_chart(df, metric):
    ...
    for model in df['Model'].unique():
        ...
        plt.plot([0, 1], [mask_value, origin_value], marker='o', label=model)
    ...
    plt.xticks([0, 1], ['Mask', 'Origin'], fontsize=12)
    plt.ylabel(f'{metric} Value')
    plt.title(f'{metric} Comparison between Mask and Origin for each Model', fontsize=14)
    plt.grid(True)
...
def plot_grouped_bar(df, metric):
    ...
    plt.title(f'{metric} Comparison between Mask and Origin', fontsize=14)
    plt.ylabel(f'{metric} Value')
    plt.grid(True)
...
def plot_radar_chart(df, model_name):
    ...
    # 성능 비교 (mask vs origin) - 색상과 스타일 조정
    ...
    # 중앙에 제목 설정
    plt.title(f'{model_name} - Radar Chart (Mask vs Origin)', size=16, color='black', y=1.1, weight='bold')
