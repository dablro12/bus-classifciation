...
class binary_model(nn.Module):
    def __init__(self, type, classes = 1):
        ...
        if type == 'default':
            ...
            with torch.no_grad():
                self.base_model.Conv2d_1a_3x3.conv.weight[:, :3, :, :] = self.base_model.Conv2d_1a_3x3.conv.weight[:, :3, :, :]  # 기존 RGB 가중치 복사
                self.base_model.Conv2d_1a_3x3.conv.weight[:, 3:, :, :].zero_()  # 새 채널은 0으로 초기화
